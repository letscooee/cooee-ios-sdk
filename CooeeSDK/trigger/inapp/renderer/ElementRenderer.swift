            ...
            if ElementType.TEXT == baseElement!.getElementType() {
                ...
                if textElement!.getCalculatedWidth() ?? 0 <= deviceWidth {
                    TextRenderer(textElement: textElement!)
                        .modifier(TextAbstractRenderer(elementData: textElement!, triggerContext: triggerContext))
                } else {
                    ZStack {}.background(
                        TextRenderer(textElement: textElement!)
                            .modifier(TextAbstractRenderer(elementData: textElement!, triggerContext: triggerContext))
                ...
                if buttonElement!.getCalculatedWidth() ?? 0 <= deviceWidth {
                    TextRenderer(textElement: buttonElement!)
                        .modifier(TextAbstractRenderer(elementData: buttonElement!, triggerContext: triggerContext))
                } else {
                    ZStack {}.background(
                        TextRenderer(textElement: buttonElement!)
                            .modifier(TextAbstractRenderer(elementData: buttonElement!, triggerContext: triggerContext))
